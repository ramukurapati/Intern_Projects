...
def process_and_write_data(raw_folder, bronze_folder, file_name, source_system):
    # Construct paths using f-strings (readability)
    raw_path = f"{paths[raw_folder]}/{current_date}/*.csv"
    bronze_folder_path = f"{paths['bronze_folder']}/{bronze_folder}/{current_date}"

    # Ensure the bronze folder exists (os.makedirs is recursive, so it's safe)
    os.makedirs(bronze_folder_path, exist_ok=True)
    ...
    row_count = df.count()
    print(f"[{source_system}] Row count: {row_count}")

    # Write path and actual writing
    file_path = f"{bronze_folder_path}/{file_name}"
    df.coalesce(1).write.csv(file_path, header=True, mode='overwrite')

    print(f"[{source_system}] Data written to: {file_path}")
